...
test("Verify dropdown contains duplicates", async ({ page }) => {
    ...
    const originaarray: string[] = [...optionsText]
    const originsortedaarray: string[] = optionsText.sort()

    console.log('Original Array', originaarray)
    console.log('Origin Sorted aarray Array', originsortedaarray)

    for(let colur of originaarray){
        console.log(colur);
    }

    expect(originaarray).toEqual(originsortedaarray)
